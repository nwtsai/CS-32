...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Sensor*> sensPtrs;
		for (auto p = auxSensors.begin(); p != auxSensors.end(); p++)
		{
			sensPtrs.push_back(&(*p));
		}

		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareSensorPtr as the ordering relationship.
		sort(sensPtrs.begin(), sensPtrs.end(), compareSensorPtr);
		sensors.clear();
		...
		for (auto p = sensPtrs.begin(); p != sensPtrs.end(); p++)
		{
			sensors.push_back(**p);
		}
