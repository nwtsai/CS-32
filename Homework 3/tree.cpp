...
{
	if (n2 <= 0) // if n2 reaches 0, that means all of a2 has been checked 
		return 1;

	else if (n1 <= 0) // if n1 reaches 0 before n2, that means not all of a2's elements were found
		return 0;

	// if the last item in each array is equal
	if (a1[n1 - 1] == a2[n2 - 1])
...
void order(double a[], int n)
{
	...
	// if there are at least two items to order before firstNotGreater, order them
	if (firstNotGreater >= 2 && firstNotGreater < n)
